...
TEST(AlgorithmFixture, heuristic_test)
{
    vector<int> blocks1 = {1, 0, 2, 3};
    vector<int> blocks2 = {1, 2, 0, 0};
    vector<int> blocks3 = {1, 1, 1, 0};
    ...
    State sorted_state = Algorithm::Heuristic(state, 1, Algorithm::quantity);
...
TEST(AlgorithmFixture, get_minimal_indexes_test)
{
    vector<int> scores = {3, 4, 8, 2, 9, 1, 5, 1};
    vector<int> indices = Algorithm::GetMinimalIndices(3, scores);

    vector<int> result = {7, 5, 3};

    ASSERT_TRUE(indices == result);
}
